...
export default function MapStatusPage() {
  ...
  // ── Admin sees ALL bins (no ward filtering) ─────────────────────────────
  const wardFilteredBins = trashBins;
  ...
  return (
    <>
      ...
      <main className="flex-grow mx-auto p-6 w-full max-w-7xl flex flex-col">
        {/* Header Section */}
        <div className="mb-8">
          <Link to="/admin" className="inline-flex items-center space-x-2 text-emerald-600 hover:text-emerald-700 transition-colors duration-300 mb-4 group">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600 text-lg">
                {viewMode === "map" 
                  ? "Real-time tracking of all waste bins" 
                  : "Overview of all waste management assets"}
              </p>
            </div>
        ...
        {wardFilteredBins.length === 0 ? (
          <div className="flex-grow flex items-center justify-center bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl border border-emerald-200 p-12" style={{ minHeight: "40vh" }}>
            <div className="text-center">
              ...
              <p className="text-gray-500 text-lg">There are currently no smart bins in the system.</p>
            </div>
          </div>
        ) : viewMode === "map" ? (
